...
#include "ISXEQSpawns.h"

// The mandatory pre-setup function.  Our name is "ISXEQSpawns", and the class is ISXEQSpawns.
...
// our instanced class.
ISXPreSetup("ISXEQSpawns",ISXEQSpawns);
...
bool ISXEQSpawns::Initialize(ISInterface *p_ISInterface)
...
WriteChatf("ISXEQSpawns Loaded");
return true;
}

// shutdown sequence
void ISXEQSpawns::Shutdown()
{
    // save settings, if you changed them and want to save them now.  You should normally save
    // changes immediately.
    //pISInterface->ExportSet(ISXEQSpawnsXML,XMLFileName);

    pISInterface->UnloadSet(ISXEQSpawnsXML);
...
void ISXEQSpawns::ConnectServices()
{
...
void ISXEQSpawns::RegisterCommands()
{
    // add any commands
...
void ISXEQSpawns::RegisterAliases()
{
    // add any aliases
}

void ISXEQSpawns::RegisterDataTypes()
{
    // add any datatypes
    // pMyType = new MyType;
...
void ISXEQSpawns::UnRegisterCommands()
{
    // remove commands
    //    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQSpawns::UnRegisterAliases()
{
    // remove aliases
}
void ISXEQSpawns::UnRegisterDataTypes()
{
    // remove data types
    //if (pMyType)
    //{
...
void ISXEQSpawns::UnRegisterTopLevelObjects()
{
    // remove Top-Level Objects
    //    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQSpawns::UnRegisterServices()
{
    // shutdown our own services
    //    if (hISXEQSpawnsService)
    //        pISInterface->ShutdownService(this,hISXEQSpawnsService);
}

void ISXEQSpawns::LoadSettings()
{
    bool bChanged=false;

    // open the settings file, and store the ID number in ISXEQSpawnsXML
    ISXEQSpawnsXML=pISInterface->OpenSettings(XMLFileName);
    ...
    Any set can be referred to by its ID.  The base set is stored in ISXEQSpawnsXML.  Note that the settings
    ...
    */
    ...
    if (unsigned int GeneralGUID=pISInterface->FindSet(ISXEQSpawnsXML,"General"))
    {
